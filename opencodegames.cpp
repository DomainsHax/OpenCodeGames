...
void OpenCodeGames::Connection(const QJsonObject jsonObjConn)
{
    ...
    if(connection_info.state() == QTcpSocket::ConnectingState) {
        ...
        if(jsonObj.value("method") == "news") {
            ...
            if(error == 1)
                error_str = "Username aldreay exist";
            else if(error == 2)
                error_str = "Email aldreay exist";
            else if(error == 3)
                error_str = "Username and Email aldreay exist";
            ...
            if(ProgressFloat >= (float)((int)ProgressFloat)+.5)
                Progress = ProgressFloat + .5;
            else
        ...
        qDebug() << "Server is not active / ConnectionError";
    }
}

void OpenCodeGames::GamesListConnect()
{
    QJsonObject jsonObj;
    jsonObj.insert("method", "games");
    jsonObj.insert("token", token);
    jsonObj.insert("os", os);
    jsonObj.insert("architecture", architecture);
    Connection(jsonObj);
}

void OpenCodeGames::PlayerStatsConnect()
{
    QJsonObject jsonObj;
    jsonObj.insert("method", "info_username");
    jsonObj.insert("token", token);
    Connection(jsonObj);
}

void OpenCodeGames::NewsInfo()
{
    QJsonObject jsonObj;
    jsonObj.insert("method", "news");
    jsonObj.insert("token", token);
    Connection(jsonObj);
}
...
void OpenCodeGames::News()
{
    ...
    panel_news->setStyleSheet("background-color: #c0c0c0");
    QScrollArea *news_scroll = new QScrollArea;
    news_scroll->setFixedSize(312, 240);
    news_scroll->setStyleSheet("border: 0px");
    ...
    for(int i = max - 1; i >= 0; i--) {
        ...
        QString creator = doc.value("creator").toString();
        QString pubdate = doc.value("pubdate").toString();
        ...
        CreatorDateLabel->setText(creator + "\n " + pubdate);
...
std::string OpenCodeGames::json_to_string(const QJsonObject jsonObj)
{
    return QString(QJsonDocument(jsonObj).toJson(QJsonDocument::Compact)).toStdString();
}
...
void OpenCodeGames::on_L_RegisterButton_clicked()
{
    StartPageDisp(2);
}

void OpenCodeGames::on_R_LoginButton_clicked()
{
    StartPageDisp(1);
}
...
void OpenCodeGames::on_button_Login_clicked()
{
    if(!LoginCompleted) {
        ...
        if(Username.isEmpty())
            ui->L_statusUsername->setText("<font color='red'>Username is empty</font>");
        else if(Username.size() < 3)
            ui->L_statusUsername->setText("<font color='red'>Username shorter than 3 chars</font>");
        else
            ui->L_statusUsername->setText("");


        if(Password.isEmpty())
            ui->L_statusPassword->setText("<font color='red'>Password is empty</font>");
        else if(Password.size() < 6)
            ui->L_statusPassword->setText("<font color='red'>Password shorter than 6 chars</font>");
        else
            ui->L_statusPassword->setText("");
    }
}
...
void OpenCodeGames::on_button_Register_clicked()
{
    if(!RegisterCompleted) {
        ...
        if(Password == RetypePassword && (!Username.isEmpty() && Username.size() >= 3) && (!Email.isEmpty()) && (!Password.isEmpty() && Password.size() >= 6) && (!RetypePassword.isEmpty() && RetypePassword.size() >= 6)) {
            QJsonObject jsonObj;
            jsonObj.insert("method", "register");
            jsonObj.insert("username", Username);
        ...
        if(Username.isEmpty())
            ui->R_statusUsername->setText("<font color='red'>Username is empty</font>");
        else if(Username.size() < 3)
            ui->R_statusUsername->setText("<font color='red'>Username shorter than 3 chars</font>");
        else
            ui->R_statusUsername->setText("");


        if(Email.isEmpty())
            ui->R_statusEmail->setText("<font color='red'>Email is empty</font>");
        else
            ui->R_statusEmail->setText("");


        if(Password.isEmpty())
            ui->R_statusPassword->setText("<font color='red'>Password is empty</font>");
        else if(Password.size() < 6)
            ui->R_statusPassword->setText("<font color='red'>Password shorter than 6 chars</font>");
        else
            ui->R_statusPassword->setText("");


        if(RetypePassword.isEmpty())
            ui->R_statusRetypePassword->setText("<font color='red'>Retype Password is empty</font>");
        else if(RetypePassword.size() < 6)
            ui->R_statusRetypePassword->setText("<font color='red'>Retype Password shorter than 6 chars</font>");
        else
            ui->R_statusRetypePassword->setText("");


        if(Password != RetypePassword)
            ui->status_Register->setText("<font color='red'>Password != Retype Password</font>");
        else
            ui->status_Register->setText("");
    }
}
...
void OpenCodeGames::CenterDisp(int disposition)
{
    ...
    if(disposition == 1)
        ui->HomeCenterWidget->setVisible(true);
    else if(disposition == 2)
        ui->SettingsCenterWidget->setVisible(true);
    else if(disposition == 3)
        ui->PlayCenterWidget->setVisible(true);
}
...
void OpenCodeGames::on_HomeButton_clicked()
{
    CenterDisp(1);
    ResetSettings();
}

void OpenCodeGames::on_SettingsButton_clicked()
{
    CenterDisp(2);
}

void OpenCodeGames::on_PlayButton_clicked()
{
    CenterDisp(3);
    ResetSettings();
}

void OpenCodeGames::on_QuitButton_clicked()
{
    close();
}
...
void OpenCodeGames::Games_List()
{
    ...
    for(int i = 1; i < max + 1; i++) {
        ...
        if(it != MapGames.end()) {
            ...
            if(DIRECTORY->exists()) {
                std::cout << "Directory exists " << it->second.dir.toStdString();

                if(VERSION->exists()) {
                    VERSION->open(QIODevice::ReadOnly);
                    QString VersRead = VERSION->readAll();
            ...
            if(max <= 4)
                it->second.GameStatus->setFixedSize(306, 83);
            else
                it->second.GameStatus->setFixedSize(288, 84);
...
void OpenCodeGames::SettingsUsername()
{
    if(ui->SUsernameValue->text().length() >= 3 && ui->SUsernameValue->text().length() <= 16) {
        QJsonObject jsonObj;
        jsonObj.insert("method", "settings_username");
        jsonObj.insert("token", token);
        jsonObj.insert("username", ui->SUsernameValue->text());
        Connection(jsonObj);
    } else {
        qDebug() << "Username < 3 chars";
    }
}
...
void OpenCodeGames::SettingsPassword()
{
    if((ui->SPasswordValue->text().length() >= 6 && ui->SPasswordValue->text().length() <= 32) && (ui->SNewPasswordValue->text().length() >= 6 && ui->SNewPasswordValue->text().length() <= 32) && (ui->SRetypeNewPasswordValue->text().length() >= 6 && ui->SRetypeNewPasswordValue->text().length() <= 32)) {
        QJsonObject jsonObj;
        jsonObj.insert("method", "settings_password");
        jsonObj.insert("token", token);
        jsonObj.insert("password", ui->SPasswordValue->text());
        jsonObj.insert("new_password", ui->SNewPasswordValue->text());
        Connection(jsonObj);
    } else {
        qDebug() << "Password < 6 chars";
    }
}
